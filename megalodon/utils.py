...
@torch.no_grad()
def check_same_in_process_group(
    x: torch.Tensor,
    name: str,
    group_name: str,
...
def init_cache(bsz, model, compute_dtype, truncate=False):
    cache_layers = [
        (
            (
                torch.empty(
                    (bsz, 0, model.local_heads, model.z_head_dim),
                    dtype=compute_dtype,
                    device="cuda",
                ),
                torch.empty(
                    (bsz, 0, model.local_heads, model.v_head_dim),
                    dtype=compute_dtype,
                    device="cuda",
                ),
                0,
...
def check_ampere_gpu():
    ...
    try:
        ...
        logger.warning(f"Error occurred while checking GPU: {e}")
